...
const ChatBotBody = () => {
    ...
    const handleInterruptClose = async () => {
        ...
        try {
            setIsLoading(true)
            const response = await fetch('/bot-message-request', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    return (
        <>
            <Snackbar className="w-full" anchorOrigin={{ vertical: "top", horizontal: "center" }} open={alertOpen} autoHideDuration={5000} onClose={() => setAlertOpen(false)}>
                <Alert onClose={() => setAlertOpen(false)} severity="success">
                    Fetch Passenger Ids Successfully.
                </Alert>
            </Snackbar>
            <div className="flex flex-col mx-40 overflow-y-auto text-white shadow-lg" style={{height: "370px"}}>
